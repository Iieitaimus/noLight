...
vert = open("swiatlo/shaders/vert.glsl", "r")
frag = open("swiatlo/shaders/frag.glsl", "r")
...
class LitObject(Entity):
    def __init__(self, model = 'plane', scale = 1, position = (0, 0, 0), rotation = (0, 0, 0), texture = None, collider = None,
        ...
        if normalMap == None: normalMap = Texture("swiatlo/textures/default_norm.png")
        if specularMap == None: specularMap = Texture("swiatlo/textures/default_spec.png")
        if water:
            normalMap = Texture("swiatlo/textures/water_norm.png")
            smoothness = 512
